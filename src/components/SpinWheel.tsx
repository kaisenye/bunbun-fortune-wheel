...
import { Prize, getPrizes, getPrizeQuantities, updatePrizeQuantity } from '@/data/prizes'
    ...
    <div className="flex flex-col items-center gap-8 w-full max-w-2xl mx-auto px-4">
      <div className="relative w-full aspect-square max-w-[500px]">
        ...
        >
          ...
          <circle cx="200" cy="200" r="40" fill="hsl(var(--accent))" stroke="white" strokeWidth="6" />
          <text x="200" y="215" textAnchor="middle" fontSize="32">
            🎁
          </text>
        </svg>
      </div>
        ...
        {prizes.map((prize) => (
          <div
            ...
            className={`p-3 rounded-lg border-2 transition-all ${
              quantities[prize.id] === 0 ? 'opacity-50 border-muted bg-muted' : 'border-primary/20 bg-card'
            }`}
          >
      ...
      {wonPrize && <PrizeModal prize={wonPrize} open={!!wonPrize} onClose={() => setWonPrize(null)} />}
    </div>
  )
}
